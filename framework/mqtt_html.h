/**
  ******************************************************************************
  * @file    mqtt_html.h
  * @author  GEEKROS,  site:www.geekros.com
  ******************************************************************************
*/

#ifndef MQTT_HTML
#define MQTT_HTML

const char mqtt_html[] PROGMEM = R"=====(

<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no">
    <title>GEEK ROS WIFI</title>
    <style>
        * {
            box-sizing: border-box;
            -webkit-tap-highlight-color: transparent;
            -webkit-appearance: none;
            -webkit-touch-callout: none;
            outline: none;
            user-select: none;
            margin: 0;
            padding: 0;
            content: none;
        }
        body {
            font-family: -apple-system,BlinkMacSystemFont,"Helvetica Neue","PingFang SC","Hiragino Sans GB","Microsoft YaHei UI","Microsoft YaHei",Arial,sans-serif;
            font-feature-settings: "tnum";
            font-variant: tabular-nums;
            -webkit-font-smoothing: antialiased;
            font-size: 14px;
            content: none;
            color: #999999;
            background-color: #f9f9f9;
        }
        a {
            text-decoration: none;
            color: #999999;
        }
        .page-view{
            width: 100%;
            height: 100%;
            margin: 0 auto;
            position: fixed;
            top: 0;
            left: 0;
            right: 0;
            bottom: 0;
            z-index: 1;
            background-color: #000000;
        }
        .page-view .return-view{
            width: 35px;
            height: 35px;
            position: absolute;
            top: 10px;
            left: 10px;
            background-color: #000000;
            border-radius: 20px;
            text-align: center;
            padding-top: 10px;
        }
        .page-view .return-view img{
            width: 15px;
            height: 15px;
        }
        .page-view .header{
            width: 100%;
            height: 200px;
            text-align: center;
            padding: 50px 0;
            background-color: #1a1c1e;
        }
        .page-view .header .logo{
            width: 60px;
            height: 60px;
            margin: 0 auto;
        }
        .page-view .header .logo img{
            width: 60px;
            height: 60px;
        }
        .page-view .header .title{
            line-height: 40px;
            font-size: 14px;
        }
        .page-view .item-view{
            width: 100%;
            padding: 40px;
        }
        .page-view .item-view .config-view{
            width: 100%;
            //display: none;
        }
        .page-view .item-view .config-view.show{
            display: block;
        }
        .page-view .item-view .config-view .item{
            width: 100%;
            height: 40px;
            margin-bottom: 10px;
        }
        .page-view .item-view .config-view .item:last-child{
            margin-top: 20px;
            text-align: center;
        }
        .page-view .item-view .config-view .item .item-select, .page-view .item-view .config-view .item .item-input{
            width: 100%;
            height: 40px;
            line-height: 40px;
            background-color: #1a1c1e;
            border: 0;
            padding: 0 10px;
            color: #999999;
            border-radius: 4px;
        }
        .page-view .item-view .config-view .item .item-select option:disabled{
            color: #333333;
        }
        .page-view .item-view .config-view .item .item-button{
            width: 200px;
            height: 40px;
            line-height: 40px;
            border-radius: 20px;
            background-color: #409EFF;
            border: 0;
            font-size: 12px;
            color: #f9f9f9;
        }
        .page-view .footer{
            width: 100%;
            height: 30px;
            position: fixed;
            left: 0;
            right: 0;
            bottom: 10px;
            font-size: 12px;
            text-align: center;
            line-height: 15px;
            color: #666666;
        }
        @media screen and (min-width: 1024px)
        {
            .page-view{
                max-width: 480px;
            }
        }
    </style>
</head>
<body onload="get_state()">
    <div class="page-view">
        <div class="return-view">
            <a href="/">
                <img src="data:image/png;base64," />
            </a>
        </div>
        <div class="header">
            <div class="logo">
                <img src="data:image/png;base64," />
            </div>
            <div class="title">GEEK ROS ESP8266 MQTT</div>
        </div>
        <div class="item-view">
            <div id="config-view" class="config-view">
				<div class="item">
                    <input type="text" id="item-ip" class="item-input" name="password" placeholder="MQTT IP">
                </div>
                <div class="item">
                    <input type="text" id="item-port" class="item-input" name="password" placeholder="MQTT Port">
                </div>
                <div class="item">
                    <button id="item-button" class="item-button" onclick="onConnect()">连接到MQTT服务器</button>
                </div>
            </div>
        </div>
        <div class="footer">
            <div class="title">Design From ESP8266-12F</div>
            <div class="title">www.geekros.com</div>
        </div>
    </div>
</body>
<script type="text/javascript">
    function get_state(){
        let ajax = new XMLHttpRequest();
        ajax.open("get","/mqtt/state");
        ajax.send();
        ajax.onreadystatechange = function () {
            if(ajax.readyState === 4 && ajax.status === 200){
                let json_data = JSON.parse(ajax.responseText);
				console.info(json_data)
				//document.getElementById("config-view").setAttribute("class", "config-view show");
            }
        };
    }
    function start_scan_connect(ip, port){
        let button = document.getElementById("item-button");
        let ajax = new XMLHttpRequest();
        ajax.open("get","/mqtt/connect?ip=" + ip + "&port=" + port);
        ajax.send();
        ajax.onreadystatechange = function () {
            if(ajax.readyState === 4 && ajax.status === 200){
                let json_data = JSON.parse(ajax.responseText);
                if(json_data.code === 0){
                    button.innerHTML = "连接成功";
                }else{
                    button.innerHTML = "连接失败，重新尝试";
                }
            }
        }
    }
    function onConnect(){
        let ip = document.getElementById("item-ip");
        let port = document.getElementById("item-port");
        let button = document.getElementById("item-button");
        if(ip.value !== "" && port.value !== ""){
            button.innerHTML = "正在连接";
            start_scan_connect(ip.value, port.value);
        }
    }
    document.body.addEventListener('touchmove', function (e) {
        e.preventDefault();
    }, {passive: false});
</script>
</html>


)=====";

#endif
